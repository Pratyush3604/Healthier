...
export default function SettingsPage() {
  ...
  const [voiceEnabled, setVoiceEnabled] = useLocalStorage('healtify-voice', true);
  const [voiceSpeed, setVoiceSpeed] = useLocalStorage('healtify-voice-speed', [1.0]);
  const [notifications, setNotifications] = useLocalStorage('healtify-notifications', true);
  const [autoRead, setAutoRead] = useLocalStorage('healtify-autoread', false);
  ...
  const handleClearData = () => {
    const keysToKeep = ['healtify-theme'];
    const themeVal = localStorage.getItem('healtify-theme');
    localStorage.clear();
    if (themeVal) localStorage.setItem('healtify-theme', themeVal);
    toast({ title: 'Data cleared', description: 'All local health data has been cleared.' });
  };
  ...
  const handleExportData = () => {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (key?.startsWith('healtify-')) data[key] = localStorage.getItem(key);
    }
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = 'healtify-data.json'; a.click();
            ...
            {tabs.map(t => {
              ...
              return (
                <button key={t.key} onClick={() => setTab(t.key)}
                  ...
                  <Icon className="w-4 h-4" />{t.label}<ChevronRight className="w-4 h-4 ml-auto" />
                </button>
              );
            })}
          </nav>
                    ...
                    {([
                      ...
                      return (
                        <button key={t.key} onClick={() => setTheme(t.key)}
                          ...
                          <Icon className="w-6 h-6" /><span className="text-sm font-medium">{t.label}</span>
                        </button>
                      );
                    })}
                  </div>
            ...
            {tab === 'voice' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                <div className="bg-card rounded-2xl p-6 border border-border space-y-6">
                  <div className="flex items-center justify-between">
                    <div><Label className="text-base font-semibold">Voice Responses</Label><p className="text-sm text-muted-foreground">Enable text-to-speech for AI responses</p></div>
                    <Switch checked={voiceEnabled} onCheckedChange={setVoiceEnabled} />
                  </div>
                  <div className="flex items-center justify-between">
                    <div><Label className="text-base font-semibold">Auto-Read Responses</Label><p className="text-sm text-muted-foreground">Automatically read AI responses aloud</p></div>
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                <div className="bg-card rounded-2xl p-6 border border-border space-y-4">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    <Button variant="outline" onClick={handleExportData} className="flex-1"><Download className="mr-2 h-4 w-4" /> Export My Data</Button>
                    <Button variant="outline" onClick={handleClearData} className="flex-1 text-warning border-warning/30 hover:bg-warning/10"><Trash2 className="mr-2 h-4 w-4" /> Clear All Data</Button>
                  </div>
                </div>
                <div className="bg-card rounded-2xl p-6 border border-border">
