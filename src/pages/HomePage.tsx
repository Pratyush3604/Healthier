...
const features = [
  { icon: Stethoscope, title: 'Symptom Checker', description: 'AI urgency assessment from 100+ symptoms', link: '/symptoms', color: 'text-success' },
  { icon: Activity, title: 'Vital Signs', description: 'Track heart rate, SpO2 & blood pressure', link: '/vitals', color: 'text-destructive' },
  { icon: Camera, title: 'Injury Detection', description: 'Upload injury photos for AI first aid', link: '/injury', color: 'text-warning' },
  { icon: Scan, title: 'Skin Analyzer', description: 'AI skin condition analysis', link: '/skin-analyzer', color: 'text-accent' },
  { icon: FileText, title: 'Report Analysis', description: 'X-rays, MRIs & labs explained', link: '/reports', color: 'text-secondary' },
  { icon: MessageCircle, title: 'AI Chat', description: 'Text-based medical consultation', link: '/chat', color: 'text-primary' },
  ...
  { icon: Lightbulb, title: 'Health Tips', description: '170+ wellness tips', link: '/health-tips', color: 'text-warning' },
  { icon: Phone, title: 'Emergency', description: '100+ country numbers', link: '/emergency', color: 'text-destructive' },
];
    ...
    <div className="min-h-screen">
        ...
        <div className="container mx-auto px-4 relative">
          <motion.div className="max-w-3xl mx-auto text-center" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            ...
            <p className="text-lg text-muted-foreground max-w-xl mx-auto mb-10 leading-relaxed">
              Instant health assessments, first aid guidance, and wellness tools.
          ...
          <div className="grid md:grid-cols-3 gap-6 max-w-4xl mx-auto">
            {howItWorks.map((step, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.12 }}
                className="bg-card rounded-2xl border border-border p-7 text-center relative">
                <div className="text-6xl font-extrabold text-muted/50 absolute top-3 right-4 select-none">{step.step}</div>
      ...
      <section className="py-20 border-t border-border">
        <div className="container mx-auto px-4">
          ...
          <motion.div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4" variants={container} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            {features.map((f, i) => {
              const Icon = f.icon;
              return (
        ...
        <div className="container mx-auto px-4">
          <motion.div className="max-w-3xl mx-auto bg-card rounded-2xl border border-border p-8" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <div className="flex items-start gap-5">
              ...
              <div>
                <h3 className="font-bold text-base mb-2">For Basic Diagnostics Only</h3>
