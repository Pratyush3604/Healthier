...
function generatePrescriptionHTML(report: HealthReport): string {
  return `<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>Healthier Report</title>
<style>
  body { font-family: 'Segoe UI', Arial, sans-serif; max-width: 700px; margin: 0 auto; padding: 40px; color: #1a1a1a; }
  .header { display: flex; align-items: center; gap: 16px; border-bottom: 3px solid #2CA3BF; padding-bottom: 16px; margin-bottom: 24px; }
  .logo { width: 50px; height: 50px; background: linear-gradient(135deg, #2CA3BF, #ACF9FD); border-radius: 12px; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; font-size: 18px; }
  .brand { font-size: 24px; font-weight: 700; color: #2CA3BF; }
  ...
  .disclaimer { margin-top: 30px; padding: 16px; background: #FFF3CD; border: 1px solid #FFECB5; border-radius: 8px; font-size: 12px; color: #664D03; }
  .footer { margin-top: 30px; padding-top: 16px; border-top: 1px solid #ddd; font-size: 11px; color: #999; text-align: center; }
  @media print { body { padding: 20px; } }
</style></head><body>
<div class="header">
  <div class="logo">H+</div>
  <div><div class="brand">Healthier</div><div class="brand-sub">AI Health Assistant</div></div>
</div>
<div class="meta"><span>Date: ${report.date}</span><span>Type: ${report.type.replace('-', ' ').toUpperCase()}</span></div>
<div class="title">${report.title}</div>
<div class="content">${report.details || report.summary}</div>
<div class="disclaimer">
  <strong>⚠️ Important Disclaimer:</strong> This report is AI-generated and for informational purposes only. It does not constitute medical advice, diagnosis, or treatment. Always consult a qualified healthcare professional for medical concerns. Healthier does not prescribe medications.
</div>
<div class="footer">Generated by Healthier — AI Health Assistant · healthier.lovable.app · © ${new Date().getFullYear()}</div>
</body></html>`;
}

function downloadAsPDF(report: HealthReport) {
  const html = generatePrescriptionHTML(report);
